...
public class FutureUtil {
     ...
     * 注意：以下代码表示：等到传入的CompletableFuture集合都完成之后，将每一个异步操作的结果收集起来，组成一个UrlInfo的列表，然后返回一个新的
     *  CompletableFuture。
     *  这只是定义了一系列的操作步骤，即先等所有的异步操作执行完成，然后再手机每一个异步操作的结果，最后返回返回一个CompletableFuture
     *  记住，这只是一个流程规划，当代码执行到这里时，所有的异步操作也许都没有执行完成，也就不会收集结果。
     *  这里thenApply中调用的join方法并不会造成阻塞。
     */
